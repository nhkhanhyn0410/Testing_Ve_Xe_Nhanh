...
Scenario(
  "TC_CANCEL_002: Tra cứu booking bằng mã đặt vé hợp lệ",
  ({ I, cancelTicketPage }) => {
    cancelTicketPage.open();
...
Scenario(
  "TC_CANCEL_003: Tra cứu booking với mã không tồn tại",
  ({ I, cancelTicketPage }) => {
    cancelTicketPage.open();
...
Scenario(
  "TC_CANCEL_004: Gửi form hủy vé với trường trống",
  ({ I, cancelTicketPage }) => {
    cancelTicketPage.open();
    cancelTicketPage.clickSearch();
    I.wait(2);
...
Scenario(
  "TC_CANCEL_005: Xem chính sách hoàn tiền sau khi tìm thấy booking",
  ({ I, cancelTicketPage }) => {
    cancelTicketPage.open();
    cancelTicketPage.fillBookingId("BK000001");
    cancelTicketPage.fillEmail(users.customer.seed.email);
