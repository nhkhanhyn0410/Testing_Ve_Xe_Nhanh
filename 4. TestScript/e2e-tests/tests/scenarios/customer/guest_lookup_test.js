...
Scenario('TC_GUEST_002: Gửi yêu cầu OTP với SĐT hợp lệ', ({ I, guestTicketLookupPage }) => {
...
Scenario('TC_GUEST_003: Xác thực OTP với mã demo 123456', ({ I, guestTicketLookupPage }) => {
  const customer = users.customer.seed;
...
Scenario('TC_GUEST_004: Xác thực OTP với mã sai', ({ I, guestTicketLookupPage }) => {
  const customer = users.customer.seed;
...
Scenario('TC_GUEST_005: Tra cứu vé bằng email', ({ I, guestTicketLookupPage }) => {
  const customer = users.customer.seed;
...
Scenario('TC_GUEST_006: Gửi yêu cầu OTP với SĐT không tồn tại', ({ I, guestTicketLookupPage }) => {
  guestTicketLookupPage.open();
...
Scenario('TC_GUEST_007: Gửi yêu cầu với trường trống', ({ I, guestTicketLookupPage }) => {
  guestTicketLookupPage.open();
  guestTicketLookupPage.clickContinue();
  I.wait(2);
