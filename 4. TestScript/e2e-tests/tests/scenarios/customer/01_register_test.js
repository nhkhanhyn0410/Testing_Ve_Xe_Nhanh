...
Scenario('TC_AUTH_010: Đăng ký thành công với thông tin hợp lệ', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();

  registerPage.open();
  registerPage.register(user.fullName, user.email, user.phone, user.password, user.password);
  I.wait(5);
...
Scenario('TC_AUTH_011: Đăng ký với email đã tồn tại', ({ I, registerPage }) => {
  ...
  registerPage.open();
  registerPage.register(user.fullName, existingUser.email, user.phone, user.password, user.password);
  I.wait(3);
  I.see(validationMessages.duplicateEmail);
});

Scenario('TC_AUTH_012: Đăng ký với email không hợp lệ', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();

  registerPage.open();
  registerPage.register(user.fullName, 'invalid-email', user.phone, user.password, user.password);
  I.wait(2);
  I.see(validationMessages.invalidEmail);
});

Scenario('TC_AUTH_013: Đăng ký với SĐT không hợp lệ (9 số)', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();

  registerPage.open();
  registerPage.register(user.fullName, user.email, '091234567', user.password, user.password);
  I.wait(2);
...
Scenario('TC_AUTH_015: Đăng ký với xác nhận mật khẩu không khớp', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();

  registerPage.open();
  registerPage.register(user.fullName, user.email, user.phone, 'Test@1234', 'DifferentPass@123');
  I.wait(2);
  I.see(validationMessages.passwordMismatch);
});

Scenario('TC_AUTH_016: Đăng ký với tất cả trường trống', ({ I, registerPage }) => {
  registerPage.open();
...
Scenario('TC_AUTH_017: Chuyển từ form đăng nhập sang đăng ký', ({ I, loginPage }) => {
  loginPage.open();
...
Scenario('TC_AUTH_018: Chuyển từ form đăng ký sang đăng nhập', ({ I, registerPage }) => {
  registerPage.open();
