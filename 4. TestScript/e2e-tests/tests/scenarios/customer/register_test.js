...
Scenario('TC_REG_001: Hiển thị form đăng ký', ({ I, registerPage }) => {
  registerPage.open();
...
Scenario('TC_REG_002: Đăng ký thành công với thông tin hợp lệ', ({ I, registerPage }) => {
...
Scenario('TC_REG_003: Đăng ký với email đã tồn tại', ({ I, registerPage }) => {
  const testData = require('../../data/users.json');
  const existingUser = testData.customer.valid;
  const user = dataGenerator.generateUser();

  registerPage.open();
  registerPage.register(user.fullName, existingUser.email, user.phone, user.password);
  I.wait(3);
  I.saveScreenshot('TC_REG_003_duplicate_email.png');
});

Scenario('TC_REG_004: Đăng ký với các trường bắt buộc để trống', ({ I, registerPage }) => {
  registerPage.open();
...
Scenario('TC_REG_005: Đăng ký với email không hợp lệ', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();
  registerPage.open();
  registerPage.register(user.fullName, 'invalid-email', user.phone, user.password);
  I.wait(2);
  I.saveScreenshot('TC_REG_005_invalid_email.png');
});

Scenario('TC_REG_006: Đăng ký với SĐT không hợp lệ', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();
  registerPage.open();
...
Scenario('TC_REG_007: Đăng ký với mật khẩu yếu', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();
  registerPage.open();
...
Scenario('TC_REG_008: Đăng ký với mật khẩu xác nhận không khớp', ({ I, registerPage }) => {
  const user = dataGenerator.generateUser();
  registerPage.open();
...
Scenario('TC_REG_009: Chuyển sang trang đăng nhập từ form đăng ký', ({ I, registerPage }) => {
  registerPage.open();
